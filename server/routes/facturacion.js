...
// ==================== CREAR FACTURA DIRECTA (servicios sin venta) ====================
router.post("/directa", async (req, res) => {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");

    const { cliente_id, tipo_comprobante, tipo_factura, dias_credito, items, notas } = req.body;

    if (!cliente_id) throw new Error("Debe seleccionar un cliente");
    if (!items || items.length === 0) throw new Error("Debe agregar al menos un servicio");

    const subtotal = items.reduce(
      (sum, i) => sum + parseFloat(i.precio || 0) * parseInt(i.cantidad || 1),
      0,
    );

    const numResult = await client.query(
      `SELECT COALESCE(MAX(REGEXP_REPLACE(numero_factura, '[^0-9]', '', 'g')::integer), 0) + 1 AS siguiente FROM facturas`,
    );
    const num = numResult.rows[0].siguiente;
    const numeroFactura = "FAC-" + String(num).padStart(8, "0");

    const ncf =
      tipo_comprobante && tipo_comprobante !== "sin_ncf"
        ? `${tipo_comprobante}-${String(num).padStart(8, "0")}`
        : null;

    const fechaVencimiento =
      tipo_factura === "credito" && dias_credito
        ? new Date(Date.now() + parseInt(dias_credito) * 24 * 60 * 60 * 1000)
            .toISOString()
            .split("T")[0]
        : null;
    ...
    const facturaResult = await client.query(
      `INSERT INTO facturas (
        numero_factura, ncf, tipo_comprobante, cliente_id,
        subtotal, descuento, itbis, total,
        fecha, fecha_vencimiento, estado, tipo_factura,
        venta_id, notas, saldo_pendiente
      ) VALUES ($1,$2,$3,$4,$5,0,0,$5,CURRENT_DATE,$6,'pendiente',$7,NULL,$8,$5) RETURNING *`,
      [
        numeroFactura,
        ncf,
        tipo_comprobante !== "sin_ncf" ? tipo_comprobante : null,
        cliente_id,
        subtotal,
        fechaVencimiento,
    ...
    for (const item of items) {
      ...
      await client.query(
        `INSERT INTO detalle_factura
          (factura_id, producto_id, nombre_producto, cantidad, precio_unitario, descuento, subtotal, itbis, total)
         VALUES ($1, NULL, $2, $3, $4, 0, $5, 0, $5)`,
        [facturaId, item.nombre, cantidad, precio, subtotalItem],
      );
    }

    await client.query("COMMIT");
    res.status(201).json({ success: true, data: facturaResult.rows[0] });
  } catch (error) {
    await client.query("ROLLBACK");
