...
.join("")}
  ...
  // ==================== FACTURA DIRECTA DE SERVICIOS ====================

  toggleFormServicios() {
    const card = document.getElementById("cardFacturaServicios");
    if (!card) return;
    const visible = card.style.display !== "none";
    if (visible) {
      this.cancelarFormServicios();
    } else {
      card.style.display = "";
      this._poblarClientesFs();
      if (!document.getElementById("fsItemsContainer").children.length) {
  ...
  cancelarFormServicios() {
    const card = document.getElementById("cardFacturaServicios");
    if (card) card.style.display = "none";
    const container = document.getElementById("fsItemsContainer");
    if (container) container.innerHTML = "";
    ["fsCliente","fsNotas","fsDiasCredito"].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.value = id === "fsDiasCredito" ? "30" : "";
    });
    const cond = document.getElementById("fsCondiciones");
    if (cond) cond.value = "contado";
    const diasGroup = document.getElementById("fsDiasCreditoGroup");
    if (diasGroup) diasGroup.style.display = "none";
    this._actualizarTotalFs();
  },

  async _poblarClientesFs() {
    const sel = document.getElementById("fsCliente");
    if (!sel || sel.options.length > 1) return;
    try {
      const clientes = await window.API.Clientes.getAll();
      clientes.forEach(c => {
        const opt = document.createElement("option");
        opt.value = c.id;
        opt.textContent = `${c.nombre}${c.apellido ? " " + c.apellido : ""}`;
        sel.appendChild(opt);
      });
    } catch (_) {}
  },

  toggleVencimientoServicios() {
    const cond = document.getElementById("fsCondiciones")?.value;
    const group = document.getElementById("fsDiasCreditoGroup");
    if (group) group.style.display = cond === "credito" ? "" : "none";
  },
  ...
  agregarFilaServicio() {
    ...
    fila.className = "fs-fila";
    fila.style.cssText = "display:grid;grid-template-columns:3fr 1fr 1fr auto;gap:8px;margin-bottom:6px;align-items:center";
    fila.innerHTML = `
      <input type="text" class="form-control fs-nombre" placeholder="Descripción del servicio" />
      <input type="number" class="form-control fs-cantidad" value="1" min="1" onchange="FacturacionModule._actualizarTotalFs()" />
      <input type="number" class="form-control fs-precio" placeholder="0.00" step="0.01" min="0" onchange="FacturacionModule._actualizarTotalFs()" oninput="FacturacionModule._actualizarTotalFs()" />
      <button type="button" class="btn btn-danger btn-small" onclick="this.closest('.fs-fila').remove();FacturacionModule._actualizarTotalFs()">✕</button>
    `;
    container.appendChild(fila);
  },

  _actualizarTotalFs() {
    const filas = document.querySelectorAll(".fs-fila");
    let total = 0;
    filas.forEach(f => {
      const qty = parseFloat(f.querySelector(".fs-cantidad")?.value) || 0;
      const precio = parseFloat(f.querySelector(".fs-precio")?.value) || 0;
      total += qty * precio;
    });
    const el = document.getElementById("fsTotalDisplay");
    if (el) el.textContent = Formatters.formatCurrency(total);
  },

  async guardarFacturaDirecta() {
    const clienteId = document.getElementById("fsCliente")?.value;
    if (!clienteId) { mostrarAlerta("Seleccione un cliente", "warning"); return; }

    const filas = document.querySelectorAll(".fs-fila");
    const items = [];
    let valido = true;
    filas.forEach(f => {
      const nombre = f.querySelector(".fs-nombre")?.value.trim();
      const cantidad = parseInt(f.querySelector(".fs-cantidad")?.value) || 1;
      const precio = parseFloat(f.querySelector(".fs-precio")?.value) || 0;
    ...
    if (!valido || items.length === 0) {
      mostrarAlerta("Complete la descripción de todos los servicios", "warning");
      return;
    }

    const tipo_factura = document.getElementById("fsCondiciones")?.value || "contado";
    const dias_credito = tipo_factura === "credito"
      ? parseInt(document.getElementById("fsDiasCredito")?.value) || 30
      : null;

    try {
      const factura = await FacturacionAPI.crearDirecta({
        cliente_id: parseInt(clienteId),
        tipo_comprobante: document.getElementById("fsComprobante")?.value || "B02",
        tipo_factura,
        dias_credito,
        items,
        notas: document.getElementById("fsNotas")?.value || null,
      });

      mostrarAlerta(`Factura ${factura.numero_factura} generada`, "success");
      this.cancelarFormServicios();

      if (window.FacturacionBusquedaModule) {
        window.FacturacionBusquedaModule.cargarFacturas();
      }
    } catch (error) {
